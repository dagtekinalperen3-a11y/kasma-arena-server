...
=====================================================================
 ...
 Elmasla kalıcı SKIN'ler al (her skinin kendi silahı, mermisi ve efekti var).
...
def fmt_hms(seconds):
    """Saniyeyi 'SS:DD:SS' biçimine çevirir (günlük market geri sayımı için)."""
...
def seconds_to_midnight():
    """Günlük görevlerin sıfırlanmasına kalan saniye (yerel gece yarısı)."""
    lt = time.localtime()
...
class SaveManager:
    DEFAULT = {
        ...
        "daily": {"date": "", "claimed_free": False, "ad_watches": 0,
                  "matches_played": 0, "match_reward_claimed": False,
                  "wave_reached": 0, "wave_reward_claimed": False,
                  "boss_killed": False, "boss_reward_claimed": False,
                  "score_beaten": False, "score_reward_claimed": False,
                  "killstreak": 0, "killstreak_reward_claimed": False,
                  "wheel_claimed": False, "streak_claimed": False},
        "login_streak": 0, "last_login_date": "", "weekly_chest_claimed_at_streak": 0,
    }
    ...
    # ---- günlük ödüller (bedava hediye / reklam / maç ödülü / +7 yeni ödül) ----
    DAILY_FREE_GEMS = 50
    DAILY_AD_GEMS = 70
    DAILY_AD_MAX = 3
    DAILY_MATCH_GOAL = 3
    ...
    STREAK_TABLE = [20, 30, 40, 55, 70, 90, 150]
    DAILY_WAVE_GOAL = 5
    DAILY_WAVE_GEMS = 45
    DAILY_BOSS_GEMS = 90
    DAILY_SCORE_GEMS = 60
    DAILY_KILLSTREAK_GOAL = 40
    DAILY_KILLSTREAK_GEMS = 40
    DAILY_WHEEL_MIN = 20
    DAILY_WHEEL_MAX = 150
    WEEKLY_CHEST_GEMS = 250

    _DAILY_DEFAULT = {"date": "", "claimed_free": False, "ad_watches": 0,
                       "matches_played": 0, "match_reward_claimed": False,
                       "wave_reached": 0, "wave_reward_claimed": False,
                       "boss_killed": False, "boss_reward_claimed": False,
                       "score_beaten": False, "score_reward_claimed": False,
    ...
    def _daily(self):
        ...
        if d.get("date") != today:
            ...
            if last_login == yesterday:
                self.data["login_streak"] = self.data.get("login_streak", 0) + 1
            elif last_login != today:
                self.data["login_streak"] = 1
            self.data["last_login_date"] = today
            for k, v in self._DAILY_DEFAULT.items():
                d[k] = v
            d["date"] = today
            # DÜZELTME: gün dönüşü + giriş serisi anında diske yazılır.
            # Eskiden yalnızca bir ödül talep edilirse kaydediliyordu; oyun
            # kapanınca seri sayacı geri kayıyordu.
            self.save()
        return d

    # -- giriş serisi (streak) --
    def login_streak_day(self):
        return max(1, self.data.get("login_streak", 1))

    def streak_reward_amount(self):
        idx = (self.login_streak_day() - 1) % len(self.STREAK_TABLE)
        return self.STREAK_TABLE[idx]

    def streak_reward_available(self):
        return not self._daily()["streak_claimed"]

    def claim_streak_reward(self):
        d = self._daily()
        if d["streak_claimed"]:
            return False
        d["streak_claimed"] = True
        self.add_gems(self.streak_reward_amount())
        self.save()
        return True

    # -- dalga hedefi --
    def register_wave_reached(self, wave):
        d = self._daily()
    ...
    def wave_reward_available(self):
        d = self._daily()
        return d["wave_reached"] >= self.DAILY_WAVE_GOAL and not d["wave_reward_claimed"]

    def claim_wave_reward(self):
        d = self._daily()
        if not self.wave_reward_available():
            return False
        d["wave_reward_claimed"] = True
        self.add_gems(self.DAILY_WAVE_GEMS)
        self.save()
        return True
    ...
    def boss_reward_available(self):
        d = self._daily()
        return d["boss_killed"] and not d["boss_reward_claimed"]

    def claim_boss_reward(self):
        d = self._daily()
        if not self.boss_reward_available():
            return False
        d["boss_reward_claimed"] = True
        self.add_gems(self.DAILY_BOSS_GEMS)
        self.save()
        return True
    ...
    def score_reward_available(self):
        d = self._daily()
        return d["score_beaten"] and not d["score_reward_claimed"]

    def claim_score_reward(self):
        d = self._daily()
        if not self.score_reward_available():
            return False
        d["score_reward_claimed"] = True
        self.add_gems(self.DAILY_SCORE_GEMS)
        self.save()
        return True

    # -- öldürme serisi (killstreak) --
    def register_killstreak(self, kills):
        d = self._daily()
    ...
    def killstreak_reward_available(self):
        d = self._daily()
        return d["killstreak"] >= self.DAILY_KILLSTREAK_GOAL and not d["killstreak_reward_claimed"]

    def claim_killstreak_reward(self):
        d = self._daily()
        if not self.killstreak_reward_available():
            return False
        d["killstreak_reward_claimed"] = True
        self.add_gems(self.DAILY_KILLSTREAK_GEMS)
        self.save()
        return True

    # -- şans çarkı --
    def wheel_reward_available(self):
        return not self._daily()["wheel_claimed"]

    def claim_wheel_reward(self):
        d = self._daily()
        if d["wheel_claimed"]:
            return 0
        d["wheel_claimed"] = True
        amount = random.randint(self.DAILY_WHEEL_MIN, self.DAILY_WHEEL_MAX)
        self.add_gems(amount)
        self.save()
        return amount

    # -- haftalık sandık (7 günlük seri) --
    def weekly_chest_available(self):
        streak = self.data.get("login_streak", 0)
        return streak > 0 and streak % 7 == 0 and self.data.get("weekly_chest_claimed_at_streak", 0) != streak

    def claim_weekly_chest(self):
        if not self.weekly_chest_available():
            return False
        self.data["weekly_chest_claimed_at_streak"] = self.data.get("login_streak", 0)
        self.add_gems(self.WEEKLY_CHEST_GEMS)
        self.save()
        return True

    def daily_free_available(self):
        return not self._daily()["claimed_free"]

    def claim_daily_free(self):
        d = self._daily()
        if d["claimed_free"]:
            return False
        d["claimed_free"] = True
        self.add_gems(self.DAILY_FREE_GEMS)
        self.save()
        return True

    def daily_ad_watches(self):
        return self._daily()["ad_watches"]

    def daily_ad_available(self):
        return self._daily()["ad_watches"] < self.DAILY_AD_MAX

    def claim_ad_reward(self):
        d = self._daily()
        if d["ad_watches"] >= self.DAILY_AD_MAX:
            return False
        d["ad_watches"] += 1
        self.add_gems(self.DAILY_AD_GEMS)
        self.save()
        return True

    def register_match_played(self):
        d = self._daily()
        if d["matches_played"] < self.DAILY_MATCH_GOAL:
            d["matches_played"] += 1
        self.save()

    def daily_matches_played(self):
        return min(self._daily()["matches_played"], self.DAILY_MATCH_GOAL)

    def daily_match_reward_available(self):
        d = self._daily()
        return d["matches_played"] >= self.DAILY_MATCH_GOAL and not d["match_reward_claimed"]

    def claim_match_reward(self):
        if not self.daily_match_reward_available():
            return False
        self._daily()["match_reward_claimed"] = True
        self.add_gems(self.DAILY_MATCH_GEMS)
        self.save()
        return True

    def daily_reward_available_any(self):
    ...
    def _weekly_chest_claimed(self):
        ...
        return (streak > 0 and streak % 7 == 0
                and self.data.get("weekly_chest_claimed_at_streak", 0) == streak)

    def daily_reward_ready_count(self):
        """Şu an talep edilebilecek toplam ödül sayısı (arayüz rozetinde gösterilir)."""
        return len(self.daily_ready_icons())

    def daily_ready_icons(self):
        """Hazır durumdaki ödüllerin ikon adları — market kartındaki mini rozetler."""
        out = []
        if self.daily_free_available():          out.append("gem")
        if self.wheel_reward_available():        out.append("orbit")
        if self.streak_reward_available():       out.append("star")
        if self.daily_ad_available():            out.append("bolt")
        if self.weekly_chest_available():        out.append("shield")
        if self.daily_match_reward_available():  out.append("target")
        if self.wave_reward_available():         out.append("boot")
        if self.boss_reward_available():         out.append("skull")
        if self.score_reward_available():        out.append("sword")
        if self.killstreak_reward_available():   out.append("fist")
        return out

    def daily_claimed_count(self):
        """Bugün kaç görevin ödülü toplandı (0-10) — ilerleme çubuğu için."""
        d = self._daily()
        n = 0
        if d.get("claimed_free"):                                 n += 1
        if d.get("ad_watches", 0) >= self.DAILY_AD_MAX:           n += 1
        if d.get("match_reward_claimed"):                         n += 1
        if d.get("streak_claimed"):                               n += 1
        if d.get("wave_reward_claimed"):                          n += 1
        if d.get("boss_reward_claimed"):                          n += 1
        if d.get("score_reward_claimed"):                         n += 1
        if d.get("killstreak_reward_claimed"):                    n += 1
    ...
    def daily_gems_available_now(self):
        ...
        total = 0
        if self.daily_free_available():         total += self.DAILY_FREE_GEMS
        if self.daily_ad_available():           total += self.DAILY_AD_GEMS * (self.DAILY_AD_MAX - self.daily_ad_watches())
        if self.daily_match_reward_available(): total += self.DAILY_MATCH_GEMS
        if self.streak_reward_available():      total += self.streak_reward_amount()
        if self.wave_reward_available():        total += self.DAILY_WAVE_GEMS
        if self.boss_reward_available():        total += self.DAILY_BOSS_GEMS
        if self.score_reward_available():       total += self.DAILY_SCORE_GEMS
        if self.killstreak_reward_available():  total += self.DAILY_KILLSTREAK_GEMS
        if self.wheel_reward_available():       total += self.DAILY_WHEEL_MIN
        if self.weekly_chest_available():       total += self.WEEKLY_CHEST_GEMS
        return total
    ...
    def owns_book(self, key):
        ...
        Kitaplar elmasla satın alınmaz; GÖREVLE açılır. Bir kitap şu üç
        durumdan biriyle açıktır:
          1) Görev şartı yok (başlangıç kitabı),
          2) Görevi tamamlanmış (istatistik hedefe ulaşmış),
          3) Daha önce açılıp kalıcı olarak kaydedilmiş.
        """
        if key in STARTER_BOOKS:
            return True
    ...
    def refresh_book_unlocks(self):
        ...
        for bk in BOOKS:
            key = bk["key"]
            if key in STARTER_BOOKS or key in owned:
                continue
...
ACHIEVEMENTS = [
    ("first_kill", "İlk Kan",           "İlk düşmanını öldür."),
    ("massacre",   "Katliam",           "Tek koşuda 250 düşman öldür."),
    ("hunter",     "Usta Avcı",         "Toplamda 2000 düşman öldür."),
    ("wave5",      "Isınma Turu",       "5. dalgaya ulaş."),
    ("wave10",     "Hayatta Kalan",     "10. dalgaya ulaş."),
    ("wave15",     "Arena Ustası",      "15. dalgaya ulaş."),
    ("wave20",     "Efsane",            "20. dalgaya ulaş."),
    ("boss",       "Dev Avcısı",        "Bir patronu yen."),
    ("combo30",    "Kombo Kralı",       "30'luk kombo yap."),
    ("lvl10",      "Tecrübeli",         "Bir koşuda 10. seviyeye ulaş."),
    ("rich",       "Altın Avcısı",      "Tek koşuda 1500 altın kazan."),
    ("bonk8",      "Yumruk Fırtınası",  "Tek BONK ile 8 düşmana vur."),
    ("cursed",     "Şeytanla Dans",     "Lanetli bir eşya satın al."),
    ("fashion",    "Modacı",            "Bir skin satın al."),
    ("nightmare",  "Kabus Avcısı",      "Kabus zorluğunda 5. dalgaya ulaş."),
    ("dasher",     "Gölge Adım",        "Tek koşuda 40 kez dash at."),
]
...
class AchievementManager:
    ...
    def draw(self, surf):
        ...
        for ach_id, life in self.toasts[:3]:
            ...
            panel(surf, r, bg=(30, 26, 14), edge=GOLD, alpha=245, radius=12)
            draw_icon(surf, r.x + 32, r.centery, "star", GOLD, 18)
            draw_text(surf, "BAŞARIM AÇILDI", (r.x + 62, r.y + 9), 12, GOLD_DIM, bold=True, shadow=False)
            draw_text(surf, a[1], (r.x + 62, r.y + 27), 19, TEXT, bold=True)
            y += h + 8
...
def shop_item_unlocked(item, current_wave):
...
BOOKS = [
    ...
    {"key": "r_bosshunter", "name": "Patron Avcısı Kitabı",
     ...
     "unlock": dict(stat="bosses", need=3, text="3 patron devir")},
    ...
    {"key": "r_lasthope", "name": "Son Umut Kitabı",
     ...
     "unlock": dict(stat="best_wave", need=15, text="15. dalgaya ulaş")},
    {"key": "r_hp_big", "name": "Dev Kitabı",
     "desc": "Azami canın %40 artar ama biraz yavaşlarsın",
     "color": (200, 140, 90), "icon": "skull", "rare": True,
     "unlock": dict(stat="best_combo", need=25, text="25'lik bir kombo yap")},
]
BOOK_BY_KEY = {b["key"]: b for b in BOOKS}
# Görev şartı olmayan kitaplar herkeste baştan açıktır.
STARTER_BOOKS = [b["key"] for b in BOOKS if not b.get("unlock")]


def book_progress(save, book):
    """Kitabın görev ilerlemesini döndürür: (şu an, hedef, tamamlandı mı).

    Görevi olmayan (başlangıç) kitaplar için (1, 1, True) döner.
    """
    unl = book.get("unlock")
    if not unl:
        return 1, 1, True
    cur = save.data.get("stats", {}).get(unl["stat"], 0) or 0
    need = unl["need"]
...
class Player:
    ...
    def try_dash(self, fx):
        ...
        self.invuln = max(self.invuln, 0.26)
        self.dash_count += 1
...
class Enemy:
    def __init__(self, kind, x, y, wave_mult, diff_mult=1.0, wave=1):
        ...
        self.kind = kind
        self.x, self.y = x, y
        self.max_hp = d["hp"] * wave_mult
        self.hp = self.max_hp
        self.speed = d["speed"] * wave_speed_mult(wave)
        dmg_mult = late_wave_dmg_mult(wave)
        self.dmg = d["dmg"] * dmg_mult * diff_mult
        self.radius = d["radius"]
        self.coin = d["coin"]
        self.xp = d["xp"]
        self.score = d["score"]
        self.contact_dps = d["contact_dps"] * dmg_mult * diff_mult
        ...
        self.hit_r = self.radius * ENEMY_HITBOX_FACTOR.get(kind, 1.0)
        self.color = ENEMY_COLORS[kind]
        self.alive = True
        self.hit_flash = 0.0
        self.spawn_t = 0.0
    ...
    def update(self, dt, player, fx, projectiles, kill_cb=None):
        ...
        if player.alive:
            ...
            if d < self.hit_r + player.radius - 2 and self.touch_cd <= 0:
                self.touch_cd = 0.12
                dealt = player.take_damage(self.contact_dps * 0.12 * 6, fx, self.x, self.y, self.kind)
...
class Boss:
    def __init__(self, kind, x, y, hp_mult, diff_mult=1.0, wave=10):
        d = BOSS_DEFS[kind]
        self.kind = kind
        self.name = d["name"]
        self.x, self.y = x, y
        ...
        dmgm = late_wave_dmg_mult(wave)
        pw_hp, pw_dmg, pw_armor = boss_power(wave)
        self.boss_index = boss_index_for_wave(wave)
        self.armor = pw_armor
...
class WaveManager:
    ...
    def is_boss_wave(self, w):
        """Patron dalgaları: 10, 15, 20, 25, 30 ..."""
    ...
    def update(self, dt, enemies, wave_mult_fn, boss_active=False, total_score=0):
        ...
        if self.spawn_timer <= 0:
            ...
            if self.wave % 5 == 0 and random.random() < elite_chance:
                x, y = self.spawn_pos()
                enemies.append(Enemy("elite", x, y, wave_mult_fn(self.wave) * 1.1, 1.0, wave=self.wave))
...
class RunState:
    ...
    def spawn_market_portal(self):
        x = random.uniform(ARENA_RECT.left + 70, ARENA_RECT.right - 70)
        y = random.uniform(ARENA_RECT.top + 70, ARENA_RECT.bottom - 70)
    ...
    def buy_shop_item(self, key):
        ...
        if not item:
            return False
        if not shop_item_unlocked(item, self.waves.wave):
            return False
    ...
    def do_storm_strike(self):
        ...
        self.fx.bolt(zigzag(e.x, ARENA_RECT.top - 20, e.x, e.y, 6, 22), (190, 215, 255), 0.3)
    ...
    def update(self, dt, input_state):
        ...
        if self.waves.boss_pending and not self.waves.boss_active:
            ...
            for i in range(n_boss):
                kind = BOSS_ORDER[self.waves.boss_idx % len(BOSS_ORDER)]
                self.waves.boss_idx += 1
                # Birden fazla patron varsa arenanın üst kısmına yatayda yayılırlar.
                if n_boss == 1:
                    bx = ARENA_RECT.centerx
                else:
                    span = min(ARENA_RECT.width - 220, 220 * (n_boss - 1))
        ...
        if self.bosses:
            ...
            if not self.bosses:
                ...
                self.waves.announce_text = f"DALGA {self.waves.wave} — YOĞUNLUK!"
                sfx("wave", 0.6, 0.0)
                if self.ach:
                    self.ach.unlock({5: "wave5", 10: "wave10", 15: "wave15",
                                     20: "wave20"}.get(self.waves.wave, ""))
        ...
        wave_changed = self.waves.update(dt, self.enemies, self.wave_hp_mult,
    ...
    def start_levelup_choice(self):
        ...
        if not pool:
            # Emniyet: hiçbir kitap yoksa başlangıç kitaplarına düş.
    ...
    def finish_run(self):
        ...
        if int(self.score) > st.get("best_score", 0):
            self.save.register_score_beaten()
        ...
        self.save.register_wave_reached(self.waves.wave)
        self.save.register_killstreak(int(self.kills))
        self.save.save()
...
def draw_arena_frame(surf):
    s = pygame.Surface((ARENA_RECT.w + 8, ARENA_RECT.h + 8), pygame.SRCALPHA)
    pygame.draw.rect(s, (*PANEL_EDGE, 160), s.get_rect(), width=3, border_radius=18)
    surf.blit(s, (ARENA_RECT.x - 4, ARENA_RECT.y - 4))
    inner = pygame.Surface((ARENA_RECT.w, ARENA_RECT.h), pygame.SRCALPHA)
    pygame.draw.rect(inner, (0, 0, 0, 40), inner.get_rect(), border_radius=14)
    surf.blit(inner, ARENA_RECT.topleft)
...
def draw_hud(surf, run, t):
    ...
    if run.bosses:
        ...
        wave_col = ORANGE if surge else (GOLD if next_is_boss else CYAN)
        head = f"DALGA {w.wave}"
        if next_is_boss:
            head += "   » SONRAKİ: PATRON"
    ...
    if run.ach:
        run.ach.draw(surf)
    run.fx.draw_texts(surf)


def draw_run(surf, run, t, aim_pos=None):
    draw_arena_frame(surf)
    prev_clip = surf.get_clip()
    surf.set_clip(ARENA_RECT)
    if run.market_portal is not None:
        run.market_portal.draw(surf, run.player)
    for hz in run.hazards:
        hz.draw(surf, t)
    for pu in run.pickups:
        pu.draw(surf, t)
    targets = sorted(run.enemies, key=lambda e: e.y)
    for e in targets:
        e.draw(surf, t)
...
class RunShopOverlay:
    ...
    def draw_and_handle(self, surf, run, mouse_pos, clicked, t):
        ...
        for i, item in enumerate(offers[:cols * max_rows_visible]):
            ...
            unlocked = shop_item_unlocked(item, run.waves.wave)
...
class App:
    ...
    def run_loop(self):
        while self.running:
            ...
            elif self.state == STATE_PLAY: self.update_play(dt, keys, mouse_pos, mouse_down, bonk_pressed, dash_pressed)
    ...
    def update_play(self, dt, keys, mouse_pos, mouse_down, bonk_pressed, dash_pressed):
        ...
        input_state["aim_x"] = mouse_pos[0]
        input_state["aim_y"] = mouse_pos[1]
        ...
        if self.run.game_over:
            if not getattr(self.run, "_daily_match_counted", False):
                self.run._daily_match_counted = True
                self.save.register_match_played()
            self.pending_board_rank = -1
            self.online_submit_status = None
    ...
    def _start_ad_watch(self):
        if not self.save.daily_ad_available() or self.ad_watch_active:
            return
        self.ad_watch_active = True
        self.ad_watch_timer = 0.0
        sfx("click", 0.6, 0.0)
    ...
    def update_daily_rewards(self, dt, mouse_pos, clicked):
        ...
        if self.ad_watch_active:
            ...
            canvas.blit(overlay, (0, 0))
            self.ad_watch_timer += dt
            remain = max(0.0, self.ad_watch_duration - self.ad_watch_timer)
            box = pygame.Rect(0, 0, 420, 220)
            box.center = (VIRTUAL_W / 2, VIRTUAL_H / 2)
            panel(canvas, box, alpha=250, edge=CYAN, edge_w=2)
            draw_text(canvas, "REKLAM OYNATILIYOR", (box.centerx, box.y + 36), 20, CYAN, bold=True, center=True)
            frac = clamp(self.ad_watch_timer / self.ad_watch_duration, 0, 1)
            bar = pygame.Rect(box.x + 40, box.y + 100, box.w - 80, 16)
            draw_bar(canvas, bar, frac, CYAN, radius=8)
            draw_text(canvas, f"{remain:0.1f} sn", (box.centerx, box.y + 140), 16, TEXT, center=True)
            draw_text(canvas, "Lütfen bekleyin...", (box.centerx, box.y + 168), 12, TEXT_DIM, center=True, shadow=False)
            if self.ad_watch_timer >= self.ad_watch_duration:
                self.ad_watch_active = False
    ...
    def _spin_wheel(self):
        ...
        if amount > 0:
            self.wheel_popup_amount = amount
            self.wheel_popup_timer = 1.8
            sfx("buy", 1.0, 0.0)
    ...
    def update_achievements(self, dt, mouse_pos, clicked, wheel_y=0):
        canvas = self.display.canvas
        self.bg.draw(canvas)
        panel_rect = pygame.Rect(0, 0, 720, 600)
        panel_rect.center = (VIRTUAL_W / 2, VIRTUAL_H / 2)
        panel(canvas, panel_rect, alpha=245)
        draw_text(canvas, "BAŞARIMLAR", (panel_rect.centerx, panel_rect.y + 34), 28, GOLD, bold=True, center=True)
        unlocked = self.save.data.get("achievements", {})
        draw_text(canvas, f"{len(unlocked)}/{len(ACHIEVEMENTS)} açıldı", (panel_rect.centerx, panel_rect.y + 66), 14, TEXT_DIM, center=True)

        cols = 2
        card_w, card_h = 320, 68
        gap = 10
        start_x = panel_rect.centerx - (cols * card_w + gap) / 2
        list_top = panel_rect.y + 92
        list_bottom = panel_rect.bottom - 62  # GERİ butonu için yer bırak
        list_h = list_bottom - list_top

        rows = math.ceil(len(ACHIEVEMENTS) / cols)
        content_h = rows * card_h + max(0, rows - 1) * gap
        max_scroll = max(0, content_h - list_h)

        list_rect = pygame.Rect(panel_rect.x, list_top, panel_rect.w, list_h)
        ...
        for i, (aid, name, desc) in enumerate(ACHIEVEMENTS):
            col_i, row = i % cols, i // cols
            y = list_top + row * (card_h + gap) - self.ach_scroll
            if y + card_h < list_top or y > list_bottom:
                continue
            rect = pygame.Rect(start_x + col_i * (card_w + gap), y, card_w, card_h)
            got = aid in unlocked
            panel(canvas, rect, bg=(30, 28, 16) if got else (20, 21, 32), edge=(GOLD if got else PANEL_EDGE), alpha=235, radius=10, edge_w=2)
            draw_icon(canvas, rect.x + 30, rect.centery, "star" if got else "shield", GOLD if got else (70, 72, 86), 15)
            draw_text(canvas, name, (rect.x + 56, rect.y + 14), 15, TEXT if got else TEXT_DIM, bold=True)
            draw_text(canvas, desc if got else "???", (rect.x + 56, rect.y + 36), 11, TEXT_DIM, shadow=False)
        canvas.set_clip(prev_clip)

        if max_scroll > 0:
            draw_scrollbar(canvas, panel_rect.right - 16, list_top, list_h, content_h, self.ach_scroll)

        btn = Button((panel_rect.centerx - 100, panel_rect.bottom - 50, 200, 40), "GERİ", lambda: self.set_state(STATE_MENU))
        btn.update(mouse_pos, dt)
        btn.draw(canvas)
        if clicked:
            btn.click(mouse_pos)
